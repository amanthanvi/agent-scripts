...
- `~/GitRepos/manager/DOMAINS.md` (domain -> target map; registrar hints; exclusions)
- `~/GitRepos/manager/DNS.md` (Cloudflare onboarding + DNS/redirect checklist)
- `~/GitRepos/manager/redirect-worker.ts` + `~/GitRepos/manager/redirect-worker-mapping.md` (worker redirects)
...
3. **Nameservers**
   - If registrar = Namecheap: `cd ~/GitRepos/manager && source profile && bin/namecheap-set-ns example.com emma.ns.cloudflare.com scott.ns.cloudflare.com`
   - If registrar = DNSimple: see `~/GitRepos/manager/DNS.md` for delegation API notes.
4. **DNS placeholders (so CF can terminate HTTPS)**
   - Proxied apex `A` + wildcard `A` → `192.0.2.1` (see `~/GitRepos/manager/DNS.md` for exact `cli4` calls).
5. **Redirect**
   - If using Page Rules: use the `cli4 --post ... /pagerules` template from `~/GitRepos/manager/DNS.md`.
   - If using Worker: update mapping (`~/GitRepos/manager/redirect-worker-mapping.md`), deploy/bind routes per `~/GitRepos/manager/DNS.md`.
6. **Verify**
...
- **Disable “Block AI bots”**: `cd ~/GitRepos/manager && source profile && bin/cloudflare-ai-bots status` / `bin/cloudflare-ai-bots disable`.

## After edits (commit/push)

If you changed anything in `~/GitRepos/manager` (docs, worker, scripts, mappings): commit there too.

1. Review: `cd ~/GitRepos/manager && git status && git diff`
