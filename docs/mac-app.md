...
- Pick bundle id: `com.amanthanvi.<appname>` (no uppercase, no spaces).
- Appcast URL: `https://raw.githubusercontent.com/amanthanvi/<Repo>/main/appcast.xml`
...
- Add to `Scripts/`:
  ...
  - `release.sh` sourcing `~/GitRepos/aman-claude-code-setup/_factory/agent-scripts/release/sparkle_lib.sh` to: lint/test, sign+notarize, clear caches, verify appcast/enclosure, optional live-update test, create GH release, check assets, tag/push.
